...
@pytest.mark.asyncio
async def test_case_runbook_management_defaults_filters_and_admin_writes(
    client: AsyncClient,
    session_maker: Any,
    admin_user_factory,
    analyst_user_factory,
) -> None:
    admin_cookie = await _login(client, session_maker, admin_user_factory, username="runbook-admin")
    analyst_cookie = await _login(client, session_maker, analyst_user_factory, username="runbook-analyst")

    create_response = await client.post(
        "/api/v1/case-runbooks",
    ...
    assert runbook["human_id"].startswith("RUN-")
    assert runbook["case_tags"] == ["dlp", "exfiltration"]
    ...
    publish_response = await client.post(
        f"/api/v1/case-runbooks/{runbook['id']}/publish",
        cookies={"intercept_session": admin_cookie},
    )
    ...
    edit_response = await client.put(
        f"/api/v1/case-runbooks/{runbook['id']}",
        cookies={"intercept_session": admin_cookie},
    ...
    disable_response = await client.post(
        f"/api/v1/case-runbooks/{runbook['id']}/disable",
        cookies={"intercept_session": admin_cookie},
    )
    ...
    disabled_list = await client.get(
        "/api/v1/case-runbooks?status=DISABLED",
        cookies={"intercept_session": analyst_cookie},
    )
    ...
    assert tombstone["runbook_tasks"] == []

    async with session_maker() as session:
        audit_events = (
            await session.execute(
                select(AuditLog.event_type)
                .where(AuditLog.entity_type == "case_runbook")
                .order_by(AuditLog.id.asc())
            )
        ).scalars().all()
        assert audit_events == [
            "case_runbook.created",
            "case_runbook.published",
            "case_runbook.updated",
            "case_runbook.disabled",
            "case_runbook.deleted",
        ]


@pytest.mark.asyncio
async def test_apply_published_runbook_creates_real_tasks_tags_and_audit_note(
    client: AsyncClient,
    session_maker: Any,
    admin_user_factory,
    analyst_user_factory,
) -> None:
    admin_cookie = await _login(client, session_maker, admin_user_factory, username="runbook-admin-apply")
    analyst_cookie = await _login(client, session_maker, analyst_user_factory, username="runbook-analyst-apply")
    ...
    apply_response = await client.post(
        f"/api/v1/case-runbooks/cases/{case_id}/apply/{runbook_id}",
        cookies={"intercept_session": analyst_cookie},
        json={
            "task_overrides": [
    ...
    async with session_maker() as session:
        ...
        assert tasks[0].source_runbook == runbook_id
        ...
        runbook_notes = [
            note
            for note in notes
            if "Applied Case Runbook" in note.get("description", "")
        ]
        assert len(runbook_notes) == 1
        assert runbook_notes[0]["created_by"] == "system"
        assert "system" in runbook_notes[0]["tags"]

        audit_events = (
            await session.execute(
                select(AuditLog.event_type).where(AuditLog.entity_type == "case_runbook")
            )
        ).scalars().all()
        assert "case_runbook.created" in audit_events


@pytest.mark.asyncio
async def test_apply_rejects_draft_runbooks_and_no_selected_tasks(
    client: AsyncClient,
    session_maker: Any,
    admin_user_factory,
    analyst_user_factory,
) -> None:
    admin_cookie = await _login(client, session_maker, admin_user_factory, username="runbook-admin-reject")
    analyst_cookie = await _login(client, session_maker, analyst_user_factory, username="runbook-analyst-reject")
    ...
    publish_response = await client.put(
        f"/api/v1/case-runbooks/{runbook_id}",
        cookies={"intercept_session": admin_cookie},
