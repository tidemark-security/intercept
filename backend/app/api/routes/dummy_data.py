...
async def populate_dummy_data(
    ...
    alerts_count: int = Query(20, ge=1, description="Number of random alerts to create (closure-prone alerts are added automatically)"),
...
async def generate_alerts_only(
    count: int = Query(10, ge=1, le=100, description="Number of random alerts to create (closure-prone alerts are added automatically)"),
    db: AsyncSession = Depends(get_db)
):
    """
...
async def get_data_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        linked_alerts_result = await db.execute(text("SELECT COUNT(*) FROM alerts WHERE case_id IS NOT NULL"))
        linked_alerts_count = linked_alerts_result.scalar()
        
        # Get status distributions
        case_status_result = await db.execute(text("SELECT status, COUNT(*) FROM cases GROUP BY status"))
        case_statuses = dict(case_status_result.fetchall())
        
        alert_status_result = await db.execute(text("SELECT status, COUNT(*) FROM alerts GROUP BY status"))
