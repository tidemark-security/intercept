...
class MCPToolsComponent(ComponentWithCache):
    ...
    def _get_server_name(self, mcp_server_value=None) -> str | None:
        """Extract the MCP server name from a component value or config payload."""
        mcp_server = mcp_server_value if mcp_server_value is not None else getattr(self, "mcp_server", None)
        if isinstance(mcp_server, dict):
            return mcp_server.get("name")
        return str(mcp_server) if mcp_server else None

    def _get_tool_names(self, tools: list | None = None) -> list[str]:
        """Return tool names for the provided tool list or the current component tools."""
        tool_list = self.tools if tools is None else tools
        return [tool.name for tool in tool_list if hasattr(tool, "name")]

    def _restore_tool_value(
        self,
        build_config: dict,
        tool_name: str,
        available_tool_names: list[str] | None = None,
    ) -> bool:
        """Restore a tool value when it is still valid for the current tool list."""
        if not tool_name:
            return False

        valid_tool_names = available_tool_names if available_tool_names is not None else self._get_tool_names()
        if valid_tool_names and tool_name not in valid_tool_names:
            return False

        build_config["tool"]["value"] = tool_name
        return True

    async def _restore_tool_selection(
        self,
        build_config: dict,
        tool_name: str,
        available_tool_names: list[str] | None = None,
        restore_dynamic_inputs: bool = False,
    ) -> bool:
        """Restore a tool and optionally rebuild its dynamic inputs."""
        restored = self._restore_tool_value(build_config, tool_name, available_tool_names)
        if not restored:
            return False

        if restore_dynamic_inputs:
    ...
    async def update_build_config(self, build_config: dict, field_value: str, field_name: str | None = None) -> dict:
        ...
        try:
            if field_name == "tool":
                ...
                saved_tool_value = (
                    build_config.get("tool", {}).get("value", "")
                    or getattr(self, "tool", "")
                )
                ...
                if not server_changed and existing_options:
                    ...
                    if not is_in_tool_mode and not use_cache:
                        ...
                        if not is_in_tool_mode:
                            build_config["tool"]["show"] = True
                        self._restore_tool_value(build_config, saved_tool_value, existing_options)
                        return build_config
                ...
                if (not cached_tools or not use_cache) and current_server_name and saved_tool_value:
                    try:
                        fetched_tools, build_config["mcp_server"]["value"] = await self.update_tool_list(field_value)
                    except (TimeoutError, asyncio.TimeoutError) as e:
                        msg = f"Timeout updating tool list: {e!s}"
                        await logger.aexception(msg)
                ...
                if not is_in_tool_mode:
                    ...
                    if cached_tools:
                        ...
                        cached_tool_names = self._get_tool_names(cached_tools)
                        build_config["tool"]["options"] = cached_tool_names
                        build_config["tool"]["placeholder"] = "Select a tool"
                    else:
                        cached_tool_names = []
                        ...
                        build_config["tool"]["options"] = []
                    restored_tool = False
                    if cached_tool_names:
                        restored_tool = await self._restore_tool_selection(
                            build_config,
                            saved_tool_value,
                            cached_tool_names,
                            restore_dynamic_inputs=True,
                        )
                    elif not server_changed and saved_tool_value:
                        restored_tool = await self._restore_tool_selection(
                            build_config,
                            saved_tool_value,
                            restore_dynamic_inputs=True,
                    ...
                    # Force a value refresh only when the current selection is unavailable.
                    if not restored_tool and (server_changed or not cached_tools or not use_cache):
                ...
                current_server_name = self._get_server_name(build_config.get("mcp_server", {}).get("value"))
                saved_tool_value = (
                    build_config.get("tool", {}).get("value", "")
                    or getattr(self, "tool", "")
                )
                ...
                if field_value:
                    ...
                    available_tool_names = self._get_tool_names()
                    if not available_tool_names and current_server_name and saved_tool_value:
                        try:
                            fetched_tools, build_config["mcp_server"]["value"] = await self.update_tool_list(
                                build_config.get("mcp_server", {}).get("value")
                            )
                        except (TimeoutError, asyncio.TimeoutError) as e:
                            msg = f"Timeout updating tool list: {e!s}"
                            await logger.aexception(msg)
                            ...
                            if fetched_tools:
                                available_tool_names = self._get_tool_names(fetched_tools)

                    build_config["tool"]["options"] = available_tool_names
                    build_config["tool"]["show"] = True
                    build_config["tool"]["placeholder"] = (
                        "Select a tool" if available_tool_names else "Loading tools..."
                    )
                    restored_tool = await self._restore_tool_selection(
                        build_config,
                        saved_tool_value,
                        available_tool_names if available_tool_names else None,
                        restore_dynamic_inputs=bool(available_tool_names),
                    )
                    if not restored_tool:
                        build_config["tool"]["value"] = uuid.uuid4()
